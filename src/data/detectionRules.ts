...
export const TECHNOLOGY_DICTIONARY: TechnologyProfile[] = [
  ...
  },
  {
    ...
    patterns: {
      headers: { 'x-hcdn-request-id': '.*', 'x-hcdn-cache-status': '.*' }
    }
  },
  ...
  },
  {
    ...
    patterns: {
      headers: { 'x-hostinger-whm': '.*', 'server': 'LiteSpeed', 'x-powered-by': 'Hostinger.*' }
    }
  },
...
export function extractTechnologyVersion(
  techSlug: string,
  signals: {
...
// Client-side / offline fallback runner
export function runDetection(
  ...
  cookies: string[] = [],
  env: string[] = []
): { tech: TechnologyProfile; matchedBy: 'headers' | 'html' | 'scripts' | 'meta' | 'cookies' | 'env'; version: string; evidence: string[] }[] {
  const matched: { tech: TechnologyProfile; matchedBy: 'headers' | 'html' | 'scripts' | 'meta' | 'cookies' | 'env'; version: string; evidence: string[] }[] = [];
  ...
  let match;
  while ((match = metaRegex.exec(normalizedHtml)) !== null) {
    metaFound[match[2].toLowerCase()] = match[3];
  }
  const metaRegex2 = /<meta[^>]+content=["']([^"']+)["'][^>]+(name|property)=["']([^"']+)["']/gi;
  while ((match = metaRegex2.exec(normalizedHtml)) !== null) {
  ...
  while ((match = scriptRegex.exec(normalizedHtml)) !== null) {
  ...
  for (const tech of TECHNOLOGY_DICTIONARY) {
    ...
    let isMatch = false;
    let matchedBy: 'headers' | 'html' | 'scripts' | 'meta' | 'cookies' | 'env' = 'html';
    const evidenceList: string[] = [];

    // 1. Headers
    if (rules.headers) {
      for (const [headerKey, pattern] of Object.entries(rules.headers)) {
        const val = normalizedHeaders[headerKey.toLowerCase()];
        if (val && new RegExp(pattern, 'i').test(val)) {
          isMatch = true;
          matchedBy = 'headers';
          evidenceList.push(`HTTP Response Header "${headerKey}: ${val}" matched pattern`);
        }
      }
    }
    ...
    if (rules.meta) {
      for (const [metaName, pattern] of Object.entries(rules.meta)) {
        const val = metaFound[metaName.toLowerCase()];
        if (val && new RegExp(pattern, 'i').test(val)) {
          isMatch = true;
          matchedBy = 'meta';
          evidenceList.push(`Meta Tag <meta name="${metaName}" content="${val}"> matched pattern`);
        }
      }
    }
    ...
    if (rules.scripts) {
      for (const pattern of rules.scripts) {
        const foundSrc = scriptsFound.find(s => new RegExp(pattern, 'i').test(s));
        if (foundSrc) {
          isMatch = true;
          matchedBy = 'scripts';
          evidenceList.push(`Script asset source "${foundSrc}" matched pattern`);
        }
      }
    }
    ...
    if (rules.html) {
      for (const pattern of rules.html) {
        if (new RegExp(pattern, 'i').test(normalizedHtml)) {
          isMatch = true;
          matchedBy = 'html';
          evidenceList.push(`HTML structure signature matched: ${pattern}`);
        }
      }
    }

    // 5. Cookies
    if (rules.cookies && cookies.length > 0) {
      for (const pattern of rules.cookies) {
        const matchedCookie = cookies.find(c => new RegExp(pattern, 'i').test(c));
        if (matchedCookie) {
          isMatch = true;
          matchedBy = 'cookies';
          evidenceList.push(`Active HTTP cookie "${matchedCookie}" matched signature`);
        }
      }
    }

    if (isMatch) {
      const version = extractTechnologyVersion(tech.slug, signalBundle);
      matched.push({
        tech,
        matchedBy,
        version,
  ...
  // Cross-Technology Dependency Resolution
  const hasWordpress = matched.some(m => m.tech.slug === 'wordpress');
  const hasMysql = matched.some(m => m.tech.slug === 'mysql');
  const hasPhp = matched.some(m => m.tech.slug === 'php');
  const hasNextjs = matched.some(m => m.tech.slug === 'nextjs');
  const hasReact = matched.some(m => m.tech.slug === 'react');

  if (hasWordpress) {
    if (!hasMysql) {
      const mysqlTech = TECHNOLOGY_DICTIONARY.find(t => t.slug === 'mysql');
      if (mysqlTech) {
        matched.push({
          tech: mysqlTech,
          matchedBy: 'env',
          version: '8.0',
          evidence: ['Implicit dependency: WordPress content engine operates on structured relational MySQL tables']
        });
      }
    }
    if (!hasPhp) {
      const phpTech = TECHNOLOGY_DICTIONARY.find(t => t.slug === 'php');
      if (phpTech) {
        matched.push({
          tech: phpTech,
          matchedBy: 'env',
          version: '8.x',
          evidence: ['Implicit dependency: WordPress core runs natively on PHP server-side scripting']
        });
      }
    }
  }
  ...
  if (hasNextjs && !hasReact) {
    const reactTech = TECHNOLOGY_DICTIONARY.find(t => t.slug === 'react');
    if (reactTech) {
      matched.push({
        tech: reactTech,
        matchedBy: 'env',
        version: '18.x',
        evidence: ['Implicit dependency: Next.js is built directly on the React component framework']
      });
    }
  }
...
export const PRESET_WEBSITES: PresetWebsite[] = [
  {
    id: 'nextjs',
    name: 'Next.js Official Portal',
    display: 'nextjs.org',
    url: 'https://nextjs.org',
    category: 'SaaS / Developer Tool',
    description: 'Vercel frontend cloud portal utilizing Next.js, React, Tailwind CSS, and Edge CDN.',
    responseHtml: '<!DOCTYPE html><html><head><title>Next.js by Vercel</title><meta name="next-head-count" content="18" /><link rel="stylesheet" href="/_next/static/css/app.css" /></head><body><div id="__next" data-next-page="/"><h1>The React Framework for the Web</h1></div><script src="/_next/static/chunks/main.js"></script><script src="https://www.googletagmanager.com/gtag/js?id=G-12345"></script></body></html>',
    responseHeaders: {
      'server': 'Vercel',
  ...
  },
  {
    ...
    display: 'techcrunch.com',
    url: 'https://techcrunch.com',
    category: 'Media / Publishing',
    description: 'Global venture and technology news publisher running on WordPress VIP, PHP, and Cloudflare.',
    responseHtml: '<!DOCTYPE html><html><head><title>TechCrunch - Startup and Technology News</title><meta name="generator" content="WordPress 6.4.3" /><link rel="stylesheet" href="/wp-content/themes/tc/style.css" /></head><body><div class="wp-block-library font-sans"><h1>TechCrunch Startup News</h1></div><script src="/wp-includes/js/jquery.js"></script><script src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script><script src="https://securepubads.g.doubleclick.net/tag/js/gpt.js"></script></body></html>',
    responseHeaders: {
      'server': 'cloudflare',
      'cf-ray': '890123456-SGP',
      'cf-cache-status': 'HIT',
  ...
  {
    id: 'gymshark',
    name: 'Gymshark E-Commerce',
    display: 'gymshark.com',
    url: 'https://gymshark.com',
    category: 'E-Commerce Storefront',
    description: 'Modern activewear storefront powered by Shopify Plus, Stripe, and Meta Pixel.',
    responseHtml: '<!DOCTYPE html><html><head><title>Gymshark Official Store</title><script src="https://cdn.shopify.com/s/files/1/0000/t/1/assets/theme.js"></script></head><body><div class="shopify-section"><button class="shopify-payment-button">Buy Now</button></div><script src="https://js.stripe.com/v3"></script><script src="https://connect.facebook.net/en_US/fbevents.js"></script></body></html>',
    responseHeaders: {
      'server': 'cloudflare',
      'x-shopid': '987654',
      'cf-cache-status': 'HIT'
  ...
  },
  {
    ...
    category: 'Design & CMS',
    description: 'High performance visual site builder using Framer React engine and Edge hosting.',
    responseHtml: '<!DOCTYPE html><html><head><title>Framer - Design and Publish</title><meta name="generator" content="Framer 2.0" /></head><body><div class="framer-website" data-framer-component-created="true"><h1>Framer</h1></div><script src="https://events.framer.com/script.js"></script></body></html>',
    responseHeaders: {
      'server': 'cloudflare',
      'cf-ray': '99887766-IAD'
    }
